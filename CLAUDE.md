...
- **네이티브 쿼리(`nativeQuery = true`)를 새로 만들면 통합 테스트를 같은 PR에 함께 낸다.** JPQL과 달리
  기동 시 검증되지 않아 그대로 통과하고 **그 화면을 열어봐야 깨진 것을 안다** — 컬럼 오타·CAST 누락·
  키셋 술어가 전부 실행 시점에야 드러난다. 테스트는 `PersistenceTest`를 상속해 실제 PostGIS에 태운다
  (`tmt-output-persistence/postgres/src/test/.../adapter/`). **키셋(커서) 쿼리는 `assertKeysetWalk`로
  `limit = 1` 순회**를 붙인다 — 정렬 키가 같은 행이 경계에 걸릴 때의 중복·누락은 그렇게만 드러난다 (TMT-348)
